...
- https://calendar-clock.vercel.app
- https://www.github.com/onitonitonito/calendar-clock.git
...
alt="플립클럭 스크린샷"
width="390"
src="./public/images/calendar-clock-01.png"/>
<img 
alt="플립클럭 스크린샷"
width="380"
src="./public/images/calendar-clock-02.png"/>
</div>
...
- **공기질 진단 및 초단기 예보**: 대기질 지수(AQI)를 9단계로 정밀 분석하고, 향후 6시간 대기질 변화를 예측하여 리포트로 제공합니다.
- **대기질 입자 시뮬레이션 (Particle Sim)**: 미세먼지 농도에 따라 배경 입자의 밀도와 흐림 정도가 동적으로 변하는 시각적 피드백을 구현했습니다.
- **캘린더 위젯**: 월별 일정 관리 및 오늘 날짜 표시 기능을 포함한 콤팩트한 달력을 제공합니다.
- **구글 캘린더 연동**: 버튼 하나로 Google Calendar와 동기화하여 다가오는 일정을 확인할 수 있습니다.
- **다국어 지원 (Bilingual Context)**: 한국어와 영어 간의 실시간 언어 전환 기능을 제공합니다.
...
- **Dynamic Visualization**: 대기질 수치에 연동된 실시간 입자 애니메이션 및 9단계 고해상도 아이콘.
- **Micro-interactions**: 컴팩트한 레이아웃과 직관적인 호버 애니메이션(물음표 커서 포인트 등).
...
### **v1.2.0 (2026-02-15)**
- **공기질 진단 시스템 고도화**: 대기질 지수(AQI)를 9단계(천상~지옥)로 정밀화하고 등급별 행동 가이드 추가.
- **초단기 예측(6H) 및 AI 리포트**: 향후 6시간 대기질 변화를 분석하여 차트와 텍스트 리포트로 제공.
- **입자 밀도 시뮬레이션**: 미세먼지 수치에 연동된 실시간 배경 입자 애니메이션 및 흐림 효과 적용.
- **인터랙션 및 UI 최적화**: 
  - 공기질 카드 전체 클릭 방식 및 물음표 커서 힌트 적용.
  - 마우스 호버 시 카드 부각(Scale up) 애니메이션 추가.
  - React Portal을 활용한 모달 레이아웃 버그 해결 및 컴팩트 대시보드 최적화.
